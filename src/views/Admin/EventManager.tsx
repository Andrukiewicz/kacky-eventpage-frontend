...
const EventManager = () => {
  ...
  const [minMapID, setMinMapID] = useState<number>(0);
  const [maxMapID, setMaxMapID] = useState<number>(0);
  ...
  const handleSave = async e => {
    ...
    if (Number.isNaN(edition)) {
      validationErrors.push('Edition must be an integer!');
    }

    if (
      Number.isNaN(minMapID) ||
      Number.isNaN(maxMapID) ||
      eventStart.length > 16 ||
      eventEnd.length > 16
    ) {
      validationErrors.push(
        'Map IDs must be integers and dates cannot exceed 16 characters.'
      );
    }
    ...
    if (
      ...
      validationErrors.push('All fields in the form are required!');
    }

    if (validationErrors.length) {
      toast({
        title: 'Validation Errors',
        description: validationErrors.join('<br/>'),
        status: 'error',
        duration: 4000,
        isClosable: true,
      });
    ...
    try {
      ...
      await setEventInfo(authentication.token, { create: createEventData });
      ...
      toast({
        title: 'Error',
        description: `Event could not be saved! ${message}`,
        status: 'error',
        duration: 4000,
        isClosable: true,
      });
