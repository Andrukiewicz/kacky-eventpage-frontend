...
interface RowData {
  getValue(key: string): number; // Define the getValue function return type
  original: { rating: number }; // Define the structure of the 'original' property
}

function mapsSort(rowA: RowData, rowB: RowData): number {
  if (
  ...
  columnHelper.accessor('difficulty', {
    ...
    cell: info => (
      <Tooltip label={info.row.original.rating} placement='end'>
        <Badge variant={diffBadgeColorArr[info.getValue()].variant}>
          &nbsp;&nbsp;
        </Badge>
      </Tooltip>
    ),
  ...
  columnHelper.accessor('number', {
    ...
    cell: info => (
      <MapNumberCell
        ...
        eventtype='hunting' // FIX PLS its 'hunting' mode or 'schedule'
      />
    ),
  }),
